...
func (a ArticleTag) GetByAID(db *gorm.DB) (ArticleTag, error) {
	...
	db.Where("article_id = ? AND is_del = ?", a.ArticleID, 0)
	if err := db.First(&articleTag).Error; err != nil && err != gorm.ErrRecordNotFound {
...
func (a ArticleTag) ListByAIDs(db *gorm.DB, articleIDs []uint32) ([]*ArticleTag, error) {
	var articleTags []*ArticleTag
	db.Where("article_id IN (?) AND is_del = ?", articleIDs, 0)
	if err := db.Find(&articleTags).Error; err != nil && err != gorm.ErrRecordNotFound {
		return nil, err
	}
	return articleTags, nil
}
